package com.fbrl.application.service;

import com.fbrl.application.port.out.LoadLatestEodSnapshotPort;
import com.fbrl.application.port.out.LoadLedgerEntriesPort;
import com.fbrl.domain.model.Account;
import com.fbrl.domain.model.EodSnapshot;
import com.fbrl.domain.model.LedgerEntry;
import com.fbrl.domain.model.Money;
import java.time.Instant;
import java.util.List;
import java.util.Optional;
import org.springframework.beans.factory.annotation.Qualifier;
import org.springframework.stereotype.Component;
import org.springframework.transaction.annotation.Transactional;

@Component
public class DemoAccountBalanceCalculator {

  private final LoadLatestEodSnapshotPort demoLoadLatestEodSnapshotPort;
  private final LoadLedgerEntriesPort demoLoadLedgerEntriesPort;

  public DemoAccountBalanceCalculator(
      @Qualifier("demo") LoadLatestEodSnapshotPort demoLoadLatestEodSnapshotPort,
      @Qualifier("demo") LoadLedgerEntriesPort demoLoadLedgerEntriesPort) {
    this.demoLoadLatestEodSnapshotPort = demoLoadLatestEodSnapshotPort;
    this.demoLoadLedgerEntriesPort = demoLoadLedgerEntriesPort;
  }

  @Transactional(value = "demoTransactionManager", readOnly = true)
  public Money calculate(Account account) {
    Optional<EodSnapshot> anchor =
        demoLoadLatestEodSnapshotPort.findLatestByAccountNumber(account.getAccountNumber());

    Money anchorBalance = anchor.map(EodSnapshot::totalBalance).orElse(Money.ZERO);
    Instant since = anchor.map(EodSnapshot::computedAt).orElse(Instant.EPOCH);

    List<LedgerEntry> entriesSinceAnchor =
        demoLoadLedgerEntriesPort.loadByAccountNumberSince(account.getAccountNumber(), since);

    return account.calculateBalance(anchorBalance, entriesSinceAnchor);
  }
}
