...
@DisplayName("DemoEodTriggerController 통합 테스트 — 온디맨드 트리거 200/409")
class DemoEodTriggerControllerTest {

  private static final String USERNAME = "demo-eod-trigger-test-admin";
  private static final String PASSWORD = "correct-password-123";
  private static final String TRIGGER_URL = "/api/v1/demo/batch-jobs/eod/trigger";
  ...
  void triggerEodSettlement_firstCall_returns200() throws Exception {
    mockMvc
  ...
  void triggerEodSettlement_secondCallSameDay_returns409() throws Exception {
    mockMvc
        .perform(post(TRIGGER_URL).header(HttpHeaders.AUTHORIZATION, bearer()))
        .andExpect(status().isOk());
  ...
  void triggerEodSettlement_withoutToken_returns401() throws Exception {
