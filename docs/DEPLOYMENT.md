...
- **`accounts` 테이블** — `DemoAccountEntity`가 데모 DB의 `accounts` 테이블에 매핑됩니다(운영 `AccountEntity`와 컬럼 구조는 동일, 완전히 별도의 물리 테이블). 데모 DB는 운영 DB와 달리 이 테이블을 자동으로 물려받지 않으므로 최초 배포 시 아래 DDL을 1회 적용할 것:
  ```sql
  ...
  ```
  안 하면 `ddl-auto: validate`가 데모 `EntityManagerFactory` 초기화 시점에 스키마 불일치를 즉시 감지해 기동 자체가 실패합니다(loud) — 운영 DB의 JPA 엔티티 스키마 불일치와 동일한 실패 양상.
